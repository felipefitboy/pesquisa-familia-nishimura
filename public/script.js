...
function answered(){const required=[...document.querySelectorAll('[data-required]')];let n=0;required.forEach(g=>{if(g.querySelector('input:checked'))n++});const total=required.length;bar.style.width=(n/total*100)+'%';ptxt.textContent=`${n} de ${total} obrigatórias respondidas`;return {n,total}}
form.addEventListener('change',answered);answered();
form.addEventListener('submit',async e=>{e.preventDefault();const err=document.querySelector('#error');err.style.display='none';const status=answered();if(status.n<status.total){err.textContent='Por favor, responda todas as perguntas obrigatórias antes de enviar.';err.style.display='block';err.scrollIntoView({behavior:'smooth',block:'center'});return}const fd=new FormData(form);const data=Object.fromEntries(fd.entries());data.sobremesas=fd.getAll('sobremesas');const btn=form.querySelector('.submit');btn.disabled=true;btn.textContent='Enviando...';try{const r=await fetch('/api/respostas',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)});if(!r.ok)throw new Error();form.hidden=true;document.querySelector('.intro').hidden=true;document.querySelector('#thanks').hidden=false;window.scrollTo({top:0,behavior:'smooth'})}catch(e){err.textContent='Não foi possível enviar agora. Tente novamente em instantes.';err.style.display='block';btn.disabled=false;btn.textContent='Enviar minha opinião'}});
